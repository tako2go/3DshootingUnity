...
public class EN_CreateBullet : MonoBehaviour
{
    ...
    public void CreateNomal(GameObject BulletType, Vector3 direction, float Speed, float size)//まっすぐ飛ぶ
    {
    ...
    public void CreateNomalHoming(GameObject BulletType, Vector3 direction, float Speed, float size)//追尾する
    {
        ...
        bullet.BulletDir = direction;//最初の方向
        bullet.EN_BulletSpeed = Speed;
        bullet.BulletSize = size;
    }

    public void CreateCircleSimultaneous(GameObject BulletType, Vector3 direction, float Speed, float size, float CircleSimRadius, int CircleSimBulletNum)//一瞬で敵の周りに円状に弾が複数現れ、同時に発射
    {
    ...
    public void CreateFan(GameObject BulletType, Vector3 direction, float Speed, float size, int bulletNum, float CneterDegree)//敵の位置から扇形のようにX-Z平面上に複数の弾が発射
    {
        ...
        for (int i = 0; i < bulletNum; i++)
        {
            ...
            Bullet.BulletSize = size;
            Bullet.BulletDir = (Quaternion.AngleAxis(StartAngle + StepAngle * i, Vector3.up) * direction.normalized);
            Bullet.EN_BulletSpeed = Speed;
            // Debug.Log(i + ":" + Quaternion.AngleAxis(StartAngle + StepAngle * i, Vector3.up) * this.transform.forward);
        }
    }
    public void CretateWave(GameObject BulletType, Vector3 direction, float Speed, float size, float Rotation, float amplitude, float frequency)
    {
        GameObject CircleWaveBullet = Instantiate(BulletType, this.transform.position, Quaternion.identity);
        EN_Wave bullet = CircleWaveBullet.AddComponent<EN_Wave>();
        bullet.BulletDir = direction;
        bullet.EN_BulletSpeed = Speed;
        bullet.BulletSize = size;
